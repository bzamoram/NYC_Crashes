...
# Data
raw_data = read.csv('Crashes_.csv', header = TRUE)
```
...
crash_data <- raw_data[sample(nrow(raw_data), size = 1000), ]
summary(crash_data)
glimpse(crash_data)
...
crash_data$CRASH.DATE <- as.Date(crash_data$CRASH.DATE, format = "%m/%d/%Y")
...
crash_data$CRASH.TIME <- as.POSIXct(crash_data$CRASH.TIME, format = "%H:%M")
time_intervals <- cut(crash_data$CRASH.TIME,
                      ...
                      labels = c("WeeHours", "Morning", "Afternoon", "Night"),
                      include.lowest = TRUE)
